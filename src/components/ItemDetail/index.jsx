...
const ItemDetail = ({ product }) => {
  ...
  const { addItem, quantityForASpecificProductInTheCart } = useContext(CartContext);
  let initial = (product.stock > quantityForASpecificProductInTheCart(product.id)) ? 1 : 0;
      ...
      <Row>
        ...
        <Col lg="8" className="py-3 border d-flex flex-column justify-content-between">
          <div>
            ...
            <span className="d-block text-end text-primary me-0"><strong>stock:</strong><i> {product.stock - quantityForASpecificProductInTheCart(product.id)}</i></span>
            <strong className={`${styles.product__price} d-block my-4`}>$ {numberFormatFn(product.price)}</strong>
          </div>
          <div>
            {(qtyAdded <= 0 && (product.stock > quantityForASpecificProductInTheCart(product.id)))
              ? <ItemCount handleAdd={handleAdd} initial={initial} stock={product.stock - quantityForASpecificProductInTheCart(product.id)} />
              : <div className='text-center'>
                ...
                <span className={(product.stock === quantityForASpecificProductInTheCart(product.id)) ? 'd-block  text-danger' : 'd-none'}>No queda stock</span>
              </div>}
            <hr />
